...
class BaseModel():
    ...
    def __init__(self, opt):
        self.opt = opt
        self.device = torch.device('cpu')#torch.device('cuda' if opt['num_gpu'] != 0 else 'cpu')
    ...
    def validation(self, dataloader, current_iter, tb_logger, save_img=True, rgb2bgr=True, use_image=True):
        """Validation function.
    ...
    def model_to_device(self, net):
        ...
        if self.opt['dist']:
            ...
            net = DistributedDataParallel(
                net,
                device_ids=[torch.current_device()],
